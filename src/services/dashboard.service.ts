...
export async function getDashboardData(params: {
  user: { id: number; role: Role; baseId: number };
  baseId?: string;
  ...
  const resolvedBaseId =
    user.role === "ADMIN" ? parseInt(baseId || "") || undefined : user.baseId;

  if (!resolvedBaseId) throw new Error("BaseId is required");
  ...
  const [purchases, transferIn, transferOut, assignments, expenditures] =
    await Promise.all([
  ...
  const netIn = 
    (purchases._sum.quantity ?? 0) + (transferIn._sum.quantity ?? 0) - (transferOut._sum.quantity ?? 0);
